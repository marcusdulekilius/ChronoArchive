...
def internet_cevap(soru):
    try:
        ...
        params = {
            ...
            'cx': CSE_ID,
            'q': soru,
            'num': 1  # En çok uyan ilk 5 sonucu al
        }
        ...
        if 'items' in data:
            for item in data['items']:
                ...
                link = item.get('link', '#')
                htmlSnippet = item.get('htmlSnippet', 'Açıklama bulunamadı')  # HTML snippet ile arama terimleri vurgulanır
                
                temizlenmis_snippet = temizle_html_etiketleri(htmlSnippet)
                result_text = f"{temizlenmis_snippet}\n\nDetaylı bilgi için:'{link}"
